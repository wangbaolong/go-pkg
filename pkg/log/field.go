...
func checkField(field Field) {
	...
	// further more, depends on the field type, we can check the actual key-value by convention
}
...
// NamedError instead of Err, use a custom key for error field
func NamedError(key string, err error) Field {
	return zap.NamedError(key, err)
}

// Err with fixed key "error"
